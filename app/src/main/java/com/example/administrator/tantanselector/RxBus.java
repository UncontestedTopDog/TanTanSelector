...
public class RxBus {
    ...
    public <T> Observable<T> register(@NonNull final Class<T> cls, final Object lifecycleObject) {
        ...
        if (lifecycleObject instanceof RxAppCompatActivity) {
            RxAppCompatActivity rxFragment
                    = (RxAppCompatActivity) lifecycleObject;
            return registerOnActivity(cls, rxFragment);
        }
    ...
    public <T> Observable<T> registerOnActivity(final Class<T> cls, final RxAppCompatActivity activity) {
        if (activity == null) {
            throw new InvalidParameterException("activity can not be null");
        }
